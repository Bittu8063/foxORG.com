...
(function ($) {
  jQuery.vimeo_player = {
    ...
    buildPlayer: function (options) {
      ...
      return this.each(function () {
        ...
        var property =
          ...
          typeof $vimeo_player.data("property") == "string"
        ...
        vimeo_player.opt.ratio =
          vimeo_player.opt.ratio == "auto" ? "16/9" : vimeo_player.opt.ratio;

        if (eval(vimeo_player.opt.loop)) vimeo_player.opt.loop = 9999;
        ...
        if (!vimeo_player.canGoFullScreen)
          vimeo_player.opt.realfullscreen = false;
        ...
        vimeo_player.opt.containment =
          vimeo_player.opt.containment == "self"
            ? jQuery(this)
        ...
        vimeo_player.canPlayOnMobile =
          vimeo_player.isSelf && jQuery(this).children().length === 0;
        ...
        vimeo_player.playerBox = jQuery("<iframe/>")
          .attr("id", playerID)
          .addClass("playerBox");
        ...
        if (!jQuery.browser.mobile || vimeo_player.canPlayOnMobile)
          ...
          if (vimeo_player.opt.mobileFallbackImage) {
            wrapper.css({
              backgroundImage:
                "url(" + vimeo_player.opt.mobileFallbackImage + ")",
        ...
        jQuery(document).on("vimeo_api_loaded", function () {
          ...
          vimeo_player.player.ready().then(function () {
            ...
            jQuery(window)
              ...
              .on("resize.vimeo_player_" + vimeo_player.id, function () {
                $vimeo_player.v_optimize_display();
              });
            ...
            vimeo_player.player.on("play", function (data) {
              ...
              if (typeof _gaq != "undefined" && eval(vimeo_player.opt.gaTrack))
                _gaq.push([
                  "_trackEvent",
                  "vimeo_player",
                  "Play",
              ...
              if (typeof ga != "undefined" && eval(vimeo_player.opt.gaTrack))
                ga(
                  "send",
                  "event",
                  "vimeo_player",
            ...
            vimeo_player.player.on("timeupdate", function (data) {
              ...
              vimeo_player.player.getPaused().then(function (paused) {
                if (paused) vimeo_player.state = 2;
              });
              ...
              if (vimeo_player.opt.showControls) {
                ...
                if (data.duration) {
                  vimeo_player.controlBar
                    ...
                    .html(
                      jQuery.vimeo_player.formatTime(data.seconds) +
                        " / " +
                        jQuery.vimeo_player.formatTime(data.duration)
                    );
                } else {
                  vimeo_player.controlBar
                    .find(".vimeo_player_time")
              ...
              if (vimeo_player.opt.addRaster) {
                var classN =
                  vimeo_player.opt.addRaster == "dot" ? "raster-dot" : "raster";
              ...
              vimeo_player.opt.stopAt =
                vimeo_player.opt.stopAt > data.duration
                  ? data.duration - 0.6
                  : vimeo_player.opt.stopAt;
    ...
    play: function () {
      ...
      if (!vimeo_player.isReady) return this;
    ...
    togglePlay: function (callback) {
      var vimeo_player = this.get(0);
      if (vimeo_player.state == 1) this.v_pause();
      else this.v_play();

      if (typeof callback == "function") callback(vimeo_player.state);
    ...
    seekTo: function (val, callback) {
      ...
      var seekTo =
        vimeo_player.opt.stopAt && val >= vimeo_player.opt.stopAt
          ? vimeo_player.opt.stopAt - 0.5
          : val;
    ...
    setVolume: function (val) {
      ...
      if (!val && !vimeo_player.opt.vol && vimeo_player.isMute)
        ...
        (!val && !vimeo_player.isMute) ||
        (val && vimeo_player.opt.vol == val)
      ) {
        if (!vimeo_player.isMute) jQuery(vimeo_player).v_unmute();
        else jQuery(vimeo_player).v_mute();
      } else {
    ...
    mute: function () {
      var vimeo_player = this.get(0);
      if (vimeo_player.isMute) return;
      vimeo_player.isMute = true;
      vimeo_player.player.setVolume(0);
      if (
        vimeo_player.volumeBar &&
        vimeo_player.volumeBar.length &&
        vimeo_player.volumeBar.width() > 10
      ) {
    ...
    unmute: function () {
      ...
      if (vimeo_player.volumeBar && vimeo_player.volumeBar.length)
        vimeo_player.volumeBar.updateSliderVal(
          vimeo_player.opt.vol > 0.1 ? vimeo_player.opt.vol : 0.1
        );
      var controls = jQuery("#controlBar_" + vimeo_player.id);
      var muteBtn = controls.find(".vimeo_player_muteUnmute");
    ...
    buildControls: function (vimeo_player) {
      ...
      if (jQuery("#controlBar_" + vimeo_player.id).length) return;
      ...
      if (
        ...
        (eval(vimeo_player.opt.realfullscreen) && !vimeo_player.isBackground)
      )
      ...
      volumeBar.simpleSlider({
        ...
        callback: function (el) {
          ...
          if (!vimeo_player.isMute) vimeo_player.opt.vol = el.value;
        },
      });
    },
    ...
    optimizeVimeoDisplay: function (align) {
      ...
      vimeo_player.opt.align =
        typeof vimeo_player.opt.align != "undefined "
          ? vimeo_player.opt.align
          : "center,center";
    ...
    fullscreen: function (real) {
      ...
      var videoWrapper = vimeo_player.isSelf
        ? vimeo_player.opt.containment
        : vimeo_player.wrapper;
